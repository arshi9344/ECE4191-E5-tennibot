...
class UltrasonicSensor:
    ...
    def __init__(self, trigger_pins=TRIGGER, echo_pins=ECHO, sensor_count=2, rotate_function=None, debug=False,
                 sensor_distance=ULTRA_DIST_BTWN, num_samples=30):
        """
    ...
    def get_distances(self, delay=0.1):
        ...
        for trigger_pin, echo_pin in zip(self.trigger_pins,
                                         self.echo_pins):  # Loop over each sensor's respective trigger and echo pins
    ...
    def detect_obstacle(self, threshold_distance):  # Purely going to be used for detecting drop off container
        ...
        obstacle_detected = any(
            distance < threshold_distance for distance in distances)  # Checking distance below threshold
        ...
        while True:
            ...
            if abs(left_distance - right_distance) < alignment_tolerance:
                ...
                if left_distance < depot_distance_threshold and right_distance < depot_distance_threshold:
                    ...
                    return ['arrived', (right_distance + left_distance) / 2]
                else:
                ...
                if right_distance < left_distance:
                    if self.debug:
                        ...
                        print(
                            f"rotating by: {angle_radians} rad which is {angle_degrees} deg")  # Call the function to rotate right
                    ...
                    if self.debug:
                        ...
                        print(
                            f"rotating by: {-angle_radians} rad which is {angle_degrees} deg")  # Call the function to rotate left
                    rotation = -angle_radians
                    return ['rotate', rotation]
