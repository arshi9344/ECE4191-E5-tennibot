...
from robot_core.utils.logging_utils import setup_logging, log_listener

"""
The input and output queues for Orchestrator:
- Shared motion data for the current instant; 
    - x, y, th
    - wl, wr
- A shared logging.log for Orchestrator to log data, including the following:
    - x, y, th
    - wl, wr
    - goal_positions
    - whether or not a ball was detected
    - current state (search, collect, return, deposit)
    - time taken for the current run
...
class Orchestrator(mp.process):
    def __init__(
        ...
        self.robot = None
        self.controller = None # PI Controller
        # self.detector = None  # We don't need this as this is going in its own process
        self.planner = None
        ...
        self.shared_data = shared_motion_data
        self.command_queue = computer_vision_data
    ...
    - 
    """
    def run(self, width=4.12, depth=5.48):
            try:
    ...
    def start(
        ...
        if self.robot is None:
            ...
            if robot is None: # If no robot is supplied, initialise a simulator robot
                print("No robot argument supplied. Initialising default (simulator robot)")
        ...
        if self.controller is None:
            ...
            if controller is None:
                print("No controller argument supplied. Initialising default PI controller.")
                self.controller = PIController(real_time=True),
            else:
        ...
        if self.planner is None:
            ...
            if planner is None:
                print("No planner argument supplied. Initialising default Tentacle Planner.")
