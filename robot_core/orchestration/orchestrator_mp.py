...
class Orchestrator(mp.Process):
    def __init__(
        ...
        print(f"Initialising Orchestrator:")
        ...
        print(f"    Initialised {controller_default} controller: Kp: {self.controller.Kp:.2f}, Ki: {self.controller.Ki:.2f}")
        ...
        print(f"    Initialised {planner_default} planner:")
        print(f"        Max linear velocity: {self.planner.max_linear_velocity:.2f}")
        print(f"        Max linear acceleration: {self.planner.max_acceleration:.2f}")
        print(f"        Max linear tolerance: {self.planner.max_linear_tolerance:.2f}")
        print(f"        Max angular velocity: {self.planner.max_angular_velocity:.2f}")
        print(f"        Max angular acceleration: {self.planner.max_angular_acceleration:.2f}")
        print(f"        Max angular tolerance: {self.planner.max_angular_tolerance:.2f}")
    ...
    def run(self):
        try:
            ...
            while self.shared_data['running']:
                ...
                elif self.shared_data['motion_state'] == RobotStates.STOP:
                    self.robot.set_motor_speed(0, 0)

                elif self.shared_data['motion_state'] == RobotStates.COLLECT:
                    # We're now collecting a ball, so insert servo control logic here
                    self.robot.set_motor_speed(0, 0)

                elif self.shared_data['motion_state'] == RobotStates.DEPOSIT:
            ...
            print("Orchestrator stopping, running Flag is false")
            self.robot.set_motor_speed(0, 0)
